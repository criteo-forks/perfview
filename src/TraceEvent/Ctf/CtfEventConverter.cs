...
namespace Microsoft.Diagnostics.Tracing.Ctf
{
    internal unsafe class CtfEventConverter : IDisposable
    {
        ...
        {
            var result = new Dictionary<string, ETWMapping>();
        ...
        private ETWMapping GetETWMapping(CtfEvent evt)
        {
            _evenMapping.TryGetValue(evt.Name, out var result);

            Debug.Assert(evt.Name.StartsWith("lttng") || _evenMapping.ContainsKey(evt.Name), evt.Name);
        ...
        private void ReleaseUnmanagedResources()
        {
            if (_header != null)
            {
                Marshal.FreeHGlobal((IntPtr)_header);
                _header = null;
            }
        }

        private struct ETWMapping
        {
            public bool IsNull => Guid == new Guid();

            public readonly Guid Guid;
            public readonly TraceEventOpcode Opcode;
            public readonly TraceEventID Id;
